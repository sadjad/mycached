...
{
  ERR_clear_error();
  const auto ret = SSL_CTX_use_certificate_file( ctx_.get(), cert_file.c_str(), SSL_FILETYPE_PEM );
...
{
  ERR_clear_error();
  const auto ret = SSL_CTX_use_PrivateKey_file( ctx_.get(), pkey_file.c_str(), SSL_FILETYPE_PEM );
